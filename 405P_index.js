...
// SECTION 2: DEBOUNCED UPDATE METHODS
	...
	initActions() {
		...
		actions['magenta_gain_set'] = {
			name: 'Magenta Gain - Set Level',
			options: [{ type: 'number', label: 'Magenta Gain (0.000-1.000)', id: 'gain_level', default: 0.500, min: 0, max: 1, step: 0.001 }],
			callback: async (event) => {
				const gainLevel = parseFloat(event.options.gain_level)
		...
		actions['yellow_gain_set'] = {
			name: 'Yellow Gain - Set Level',
			options: [{ type: 'number', label: 'Yellow Gain (0.000-1.000)', id: 'gain_level', default: 0.500, min: 0, max: 1, step: 0.001 }],
			callback: async (event) => {
				const gainLevel = parseFloat(event.options.gain_level)
		...
		feedbacks['magenta_gain_level'] = {
			type: 'boolean',
			name: 'Magenta Gain - Level Check',
			description: 'Change button appearance based on magenta gain level', 
			defaultStyle: { bgcolor: 0xFF00FF, color: 0xFFFFFF },
			options: [
				{ type: 'dropdown', label: 'Condition', id: 'condition', default: 'equal', choices: [
					{ id: 'equal', label: 'Equal to' }, { id: 'greater', label: 'Greater than' }, { id: 'less', label: 'Less than' }
				]},
				{ type: 'number', label: 'Magenta Gain Level', id: 'gain_level', default: 0.500, min: 0, max: 1, step: 0.001 }
			],
			callback: (feedback) => {
		...
		feedbacks['yellow_gain_level'] = {
			type: 'boolean',
			name: 'Yellow Gain - Level Check',
			description: 'Change button appearance based on yellow gain level', 
			defaultStyle: { bgcolor: 0xFFFF00, color: 0x000000 },
			options: [
				{ type: 'dropdown', label: 'Condition', id: 'condition', default: 'equal', choices: [
					{ id: 'equal', label: 'Equal to' }, { id: 'greater', label: 'Greater than' }, { id: 'less', label: 'Less than' }
				]},
				{ type: 'number', label: 'Yellow Gain Level', id: 'gain_level', default: 0.500, min: 0, max: 1, step: 0.001 }
			],
			callback: (feedback) => {
